...
| `git reset HEAD~qty --hard`            | `git reset HEAD~1 --hard`            | Откат состояния репозитория на один коммит назад с изменением файлов                            |
| `git reset ORIG_HEAD --hard`            | `git reset ORIG_HEAD --hard`             | Позволяет вернуться к предыдущему коммиту после `git reset HEAD~qty --hard`                      |
