...
### Шпаргалка: Откат изменений в Git
...
|                           | `git reset`                   | `git reset`                        | Сбросить индекс (эквивалент `git restore --staged .`)                    | Быстрая очистка индекса |
|                           | `git reset файл`              | `git reset файл`                   | Убрать конкретный файл из индекса                                        | Точечное удаление из индекса |
...
|                           | `git reset --hard`            | `git reset --hard HEAD~1`          | Полный откат к предыдущему коммиту                                       | Полностью отказаться от изменений |
...
|                           | `git reset ORIG_HEAD`         | `git reset ORIG_HEAD --hard`       | Вернуться к состоянию до последнего опасного действия                    | После ошибочного `reset --hard` |

### ⚠️ Важные предупреждения:
- **`git reset --hard`** — безвозвратно удаляет все незакоммиченные изменения!
- **`git reflog`** — показывает историю всех действий для восстановления
- **`ORIG_HEAD`** — автоматически сохраняет состояние перед опасными операциями
